...
`vout` 為 UTXO output index。BTC 等 UTXO 鏈一筆交易可能含多個 output，僅 `tx_hash` 不足以定位受益人收到的 output，故新增此欄位。僅在 `network` 為 UTXO 鏈（如 `bitcoin`）時提供；非 UTXO 鏈可省略或為 `null`。
</Note>
...
鏈以 [CAIP-2](https://chainagnostic.org/CAIPs/caip-2) `chain_id`（格式 `namespace:reference`）為**正規識別碼**，填入 `transaction.chain_id`，與國際錢包/協定生態對齊並消除冗長自訂清單。短名 `network`（如 `ethereum`）於**過渡期保留為別名**，完整切換為 CAIP-only 規劃於 v2.3。
</Note>
...
| Tron | tron | （待 CAIP namespace 註冊） | **待核對**（Tron namespace 尚未併入 CAIP registry，暫沿用短名 `tron`） |
...
資產以 [CAIP-19](https://chainagnostic.org/CAIPs/caip-19) `asset_id` 為**正規識別碼**（填入 `transaction.asset_id`），人類可讀代號另存於 `transaction.asset`（token symbol，如 `USDT`）。**凡可由 CAIP-19 表示、且位於上述支援 CAIP-2 鏈上的資產皆受支援，不再逐欄列舉。** 下表僅列常見資產的標準範例。
