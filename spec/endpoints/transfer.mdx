...
| transaction.chain_id | string | 建議 | **[v2.2 新增]** CAIP-2 chain_id（如 `eip155:1`），鏈的正規識別碼，見[資料模型](/spec/data-models) |
| transaction.asset | string | Y | 資產代號 token symbol（如 `ETH`、`USDT`） |
| transaction.asset_id | string | 建議 | **[v2.2 新增]** CAIP-19 asset_id（如 `eip155:1/erc20:0x...`），資產的正規識別碼，可精確區分 wrapped/bridged |
...
`transfer_id` 由**發起方 VASP 自行定義**，不再強制採用 `tr_{yyyyMMDD}_{流水號}` 格式（流水號在跨 VASP 並發時易碰撞）。

- **長度上限 36 字元**（與 UUID 等長）
- 在發起方 VASP 範圍內唯一即可；因 API 請求 Header 已帶 `X-VASP-ID`，故 `(X-VASP-ID, transfer_id)` 組合即為全域唯一
- **建議**採用 UUID 或 ULID 等隨機識別碼（不可預測、含時間序）；範例：`tr_01HV5K8XYZP3QRT4ABCD1234EF`
</Note>
